...
let buildProjs = !! "src/**/*.*proj" ++ "tests/**/*.*proj"
...
let commonBuildOpts = { commonDotNetOpts with
                            CustomParams = Some (
                                            (sprintf
                                            "/p:BaseIntermediateOutputPath=%s/ /p:IntermediateOutputPath=%s/"
                                            builOutAbs builOutAbs
                                            )
                            )
                       }
...
let getChangelogVersion () =
    ...
    with excp
        | excp -> (Trace.traceError (sprintf "Error '%s' parsing changelog '%s'"
                                                excp.Message changelogPath
                                    );
...
let getVersion (changelogVers:SemVerInfo option) (argVersion:SemVerInfo) =
    ...
    | Some vers -> if vers <> argVersion then
                    failwith (sprintf
                "Error: The version in the Changelog is %s from command line is %s"
                        vers.AsString argVersion.AsString) else vers.AsString
